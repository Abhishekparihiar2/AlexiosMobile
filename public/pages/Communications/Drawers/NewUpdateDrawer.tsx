...
import { X, Upload, Globe, Building2, MapPin, Users, CheckSquare } from 'lucide-react';

interface NewUpdateDrawerProps {
  isOpen: boolean;
  onClose: () => void;
}

export function NewUpdateDrawer({ isOpen, onClose }: NewUpdateDrawerProps) {
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full max-w-[600px] bg-slate-50 shadow-2xl z-40 flex flex-col animate-in slide-in-from-right duration-300 dark:bg-slate-900">
        <div className="px-6 py-5 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 dark:bg-slate-900 dark:border-slate-700">
          <div className="flex flex-col gap-1">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Create Operational Update</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Send an operational update and track acknowledgements.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
          ...
          <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm dark:bg-slate-900 dark:border-slate-700">
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Update Title <span className="text-red-500">*</span></label>
              <input 
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="e.g. Protocol Change for Entry Gate"
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Update Details <span className="text-red-500">*</span></label>
              <textarea 
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                placeholder="Describe the operational changes or updates..."
                rows={6}
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1.5 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
                  <CheckSquare className="w-3.5 h-3.5 text-slate-400" /> Require Acknowledgement
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm dark:bg-slate-900 dark:border-slate-700">
            <label className="text-sm font-bold text-slate-800 dark:text-slate-200">Target Audience <span className="text-red-500">*</span></label>
